...
func recordPath(x int, y int, dist int) {
		if game_board[x] == nil {
...
func checkCollision(x int, y int, dist int) {
		if x == 0 && y == 0 {
				return
		}
		if game_board[x] != nil && game_board[x][y] != 0 {
				fmt.Printf("%d\n", game_board[x][y] + dist) //mathAbs(x) + mathAbs(y))	
		}
}

func wire_two(wirePath []string, myFunc func(int, int, int)) {
		x := 0
		y := 0
		dist := 0
