...
var natList map[string]bool

var mQueue [50][]inputState
var regList [50]bool
...
func input(address int) int {
	if !regList[address] {
		regList[address] = true
		return address
	}
	...
	if idleCount == 50 {
		...
		if nat != nil {
			...
			fmt.Println("get natted")

			var buf bytes.Buffer

			fmt.Fprintf(&buf, "%v, %v", nat.x, nat.y)
	...
	/*
		if mQueue[address] == nil {
			mQueue[address] = make([]inputState, 0)
			fmt.Printf("Registered machine %v!\n", address)
			return address
		} */

	// get input
	if len(mQueue[address]) == 0 {
		//fmt.Printf("gave to %v\n", address)
		idling[address] = true
		return -1
	}
	...
	if !mQueue[address][0].xRead {
		mQueue[address][0].xRead = true
		return mQueue[address][0].x
	}

	mQueue[address][0].yRead = true
	retVal := mQueue[address][0].y
	// pop
	mQueue[address] = mQueue[address][1:]
...
var outputStates [50]*outputState
...
func send(data *outputState) {
	...
	mQueue[data.dest] = append(mQueue[data.dest], newVal)
}

func output(val, machineName int) {
	// get curr state

	currState := outputStates[machineName]

	if currState == nil {
		currState = newOutputState()
	}

	if currState.destNil {
		currState.destNil = false
		currState.dest = val
		outputStates[machineName] = currState
	} else if currState.xNil {
		currState.xNil = false
		currState.x = val
		outputStates[machineName] = currState
	} else {
		currState.y = val
		currState.yNil = false
		send(outputStates[machineName])
		outputStates[machineName] = nil

	}
}

func run(mach *intcode.IntcodeState, wg *sync.WaitGroup) {
	mach.Loop()
	wg.Done()
}

func main() {
	// mQueue = make([51][]message, 51)
